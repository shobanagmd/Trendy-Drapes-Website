                ...
                filteredCoupons.map((coupon) => (
                  <tr key={coupon.coupon_id} className="hover:bg-secondary/10 transition-colors">
                    ...
                    <td className="px-6 py-4 text-xs">
                      {coupon.seller_name || coupon.seller_id ? (
                        <div className="flex flex-col">
                          <span className="badge-info text-[9px] px-2 py-0.5 w-fit mb-1">SELLER</span>
                          <span className="font-semibold text-card-foreground">{coupon.seller_name || "Marketplace Seller"}</span>
                        </div>
                      ) : (
                        <div className="flex flex-col">
                          <span className="badge-neutral text-[9px] px-2 py-0.5 w-fit mb-1 text-muted-foreground">ADMIN</span>
                          <span className="font-semibold text-muted-foreground italic">Platform-wide</span>
                        </div>
                      )}
                    </td>
                    <td className="px-6 py-4">
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <div className="h-1.5 w-24 bg-secondary rounded-full overflow-hidden">
                          <div 
                            ...
                            style={{ width: `${coupon.max_usage ? Math.min((coupon.used_count / coupon.max_usage) * 100, 100) : 0}%` }}
                          />
                        </div>
                        <div className="text-[9px] text-muted-foreground mt-0.5">
                          Per User Limit: {coupon.max_usage_per_user || 1}
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
          ...
          <div className="bg-card border rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleCreateCoupon} className="p-6 space-y-4">
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div>
                  <label className="block text-xs font-bold text-muted-foreground uppercase mb-1.5 ml-1">Max Per User</label>
                  <Input 
                    type="number" 
                    placeholder="1"
                    value={newCoupon.max_usage_per_user}
                    onChange={(e) => setNewCoupon({...newCoupon, max_usage_per_user: e.target.value})}
                  />
                </div>
              </div>
