...
const PIE_DATA = [
  { name: "Gold", value: 38, color: "#C9A14A" },
  { name: "Diamond", value: 28, color: "#93c5fd" },
  { name: "Silver", value: 18, color: "#94a3b8" },
  { name: "Platinum", value: 10, color: "#e2e8f0" },
  { name: "Gemstone", value: 6, color: "#86efac" },
];
...
const avatarBg = (str) => {
  const colors = ["#C9A14A","#93c5fd","#86efac","#f9a8d4","#a78bfa","#34d399"];
    ...
    <div style={{ ...styles.root, background: D.bg, color: D.text, minHeight: "100vh" }}>
      ...
      <aside style={{ ...styles.sidebar, background: D.card, borderRight: `1px solid ${D.border}`, transform: sidebarOpen ? "translateX(0)" : "translateX(-100%)", boxShadow: "4px 0 24px rgba(0,0,0,0.06)" }}>
        <div style={styles.sidebarLogo}>
          <div style={{ ...styles.logoMark, background: "linear-gradient(135deg, #C9A14A, #f0c97a)" }}>
            <Award size={18} color="#fff" />
          </div>
          {sidebarOpen && (
            <div onClick={() => navigate("/")} style={{ cursor: "pointer" }}>
              <div style={{ fontWeight: 700, fontSize: 16, color: "#C9A14A", letterSpacing: "-0.3px" }}>Trendy Drapes</div>
              <div style={{ fontSize: 11, color: D.muted, marginTop: 1 }}>Seller Center</div>
            </div>
          )}
        </div>
              ...
              {profileOpen && (
                <div style={{ ...styles.dropdown, background: D.card, border: `1px solid ${D.border}`, right: 0, width: 200 }}>
                  {[{ icon: User, label: "My Profile" }, { icon: LogOut, label: "Sign Out" }].map(({ icon: Icon, label }) => (
                    <button key={label} onClick={() => { if (label === "My Profile") setPage("settings"); setProfileOpen(false); }} style={{ ...styles.dropItem, color: label === "Sign Out" ? "hsl(var(--destructive))" : "hsl(var(--foreground))" }}>
                      <Icon size={15} /> {label}
                    </button>
                  ))}
                </div>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          <div>
            ...
            {loading ? <Skeleton w={160} h={36} /> : (
              <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
                <span style={{ fontSize: 28, fontWeight: 700, color: "#C9A14A" }}>{fmtShort(totalRevenue)}</span>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            {["weekly", "yesterday", "monthly"].map(f => (
              <button key={f} onClick={() => setRevenueFilter(f)} style={{ ...styles.filterBtn, background: revenueFilter === f ? "#C9A14A" : D.bg, color: revenueFilter === f ? "#fff" : D.muted, border: `1px solid ${revenueFilter === f ? "#C9A14A" : D.border}` }}>
        ...
        {loading ? <Skeleton w="100%" h={220} r={12} /> : (
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={revenueData || []} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
              ...
              <Area type="monotone" dataKey="revenue" stroke="#C9A14A" strokeWidth={2.5} fill="url(#goldGrad)" dot={false} activeDot={{ r: 5, fill: "#C9A14A" }} />
            </AreaChart>
          </ResponsiveContainer>
        )}
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <div style={{ ...styles.card(D), padding: 24 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 20 }}>
            <TrendingUp size={18} color="#C9A14A" />
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {topProducts.map((p, i) => (
              <div key={p?.id || i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px", background: D.bg, borderRadius: 10 }}>
                <span style={{ width: 28, height: 28, borderRadius: 8, background: i === 0 ? "#C9A14A" : i === 1 ? "#d4af37" : D.border, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, color: i < 2 ? "#fff" : D.muted }}>{i + 1}</span>
                ...
                <div style={{ fontSize: 13, fontWeight: 700, color: "#C9A14A" }}>{fmtShort(p?.price || 0)}</div>
              </div>
            ))}
          </div>
          <div style={{ marginTop: 20, paddingTop: 16, borderTop: `1px solid ${D.border}`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 700, color: "#C9A14A" }}>{topProducts.reduce((sum, p) => sum + (p?.sold || 0), 0)}</div>
          </div>
        </div>
      </div>
    );
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          ...
          {filterCategories.map(c => (
            <button 
              ...
              style={{ 
                ...styles.filterBtn, 
                background: catFilter === c ? "#C9A14A" : D.bg, 
                color: catFilter === c ? "#fff" : D.muted, 
                border: `1px solid ${catFilter === c ? "#C9A14A" : D.border}` 
              }}
            >
              {c}
            </button>
            ...
            <tbody>
              {filtered.map((p, i) => p && (
                <tr key={`row-${p.id || 'new'}-${i}`} style={{ borderBottom: `1px solid ${D.border}` }}>
                  ...
                  <td style={{ padding: "14px 16px" }}>
                    <span style={{ fontSize: 12, background: "rgba(201,161,74,0.1)", color: "#C9A14A", padding: "3px 10px", borderRadius: 20 }}>
                      {p.category || "General"}
                    </span>
                  </td>
                  ...
                  <td style={{ padding: "14px 16px" }}>
                    <div style={{ display: "flex", gap: 6 }}>
                      <button onClick={() => openEdit(p)} style={{ ...styles.iconBtn, background: "rgba(201,161,74,0.1)", width: 30, height: 30 }}><Edit2 size={13} color="#C9A14A" /></button>
          ...
          {filtered.map((p, i) => p && (
            <div key={`grid-${p.id || 'new'}-${i}`} style={{ ...styles.card(D), padding: 20, transition: "transform 0.2s", display: "flex", flexDirection: "column" }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 18, fontWeight: 700, color: "#C9A14A", marginBottom: 16 }}>
                  {fmt(p.price || 0)}
                </div>
              </div>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "flex", gap: 12 }}>
        {statuses.map(s => {
          ...
          return (
            <button key={s} onClick={() => setStatusFilter(s)} style={{ ...styles.card(D), padding: "12px 20px", display: "flex", gap: 8, alignItems: "center", cursor: "pointer", border: statusFilter === s ? "2px solid #C9A14A" : `2px solid ${D.border}`, flex: 1, justifyContent: "center" }}>
              <span style={{ fontSize: 20, fontWeight: 700, color: statusFilter === s ? "#C9A14A" : D.text }}>{count}</span>
              ...
              {filtered.map((o, i) => {
                ...
                return (
                  <tr key={`order-row-${o.id || i}`} style={{ borderBottom: `1px solid ${D.border}` }} onMouseEnter={e => e.currentTarget.style.background = D.bg} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                    <td style={{ padding: "14px 16px", fontSize: 13, fontWeight: 600, color: "#C9A14A" }}>{o.id}</td>
                    <td style={{ padding: "14px 16px" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {["weekly", "monthly", "yearly"].map(f => (
            <button key={f} onClick={() => setDateRange(f)} style={{ ...styles.filterBtn, background: dateRange === f ? "#C9A14A" : D.card, color: dateRange === f ? "#fff" : D.muted, border: `1px solid ${dateRange === f ? "#C9A14A" : D.border}` }}>
              {f.charAt(0).toUpperCase() + f.slice(1)}
            </button>
          ))}
        </div>
      ...
      <div style={{ ...styles.card(D), padding: "24px" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          <div>
            ...
            {loading ? <Skeleton w={160} h={36} /> : (
              <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
                <span style={{ fontSize: 28, fontWeight: 700, color: "#C9A14A" }}>{fmtShort(revTotal)}</span>
        ...
        {loading ? <Skeleton w="100%" h={220} r={12} /> : (
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={revenueData || []} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
              ...
              <YAxis tick={{ fontSize: 11, fill: D.muted }} axisLine={false} tickLine={false} tickFormatter={fmtShort} width={55} />
              <Tooltip content={<CustomTooltip D={D} />} />
              <Area type="monotone" dataKey="revenue" stroke="#C9A14A" strokeWidth={2.5} fill="url(#analyticsGoldGrad)" dot={false} activeDot={{ r: 5, fill: "#C9A14A" }} />
            </AreaChart>
          </ResponsiveContainer>
        )}
      </div>
            ...
            {loading ? <Skeleton h={220} r={10} /> : chartData.length > 0 ? (
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={chartData} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                  ...
                  <YAxis tick={{ fontSize: 11, fill: D.muted }} axisLine={false} tickLine={false} tickFormatter={fmtShort} width={55} />
                  <Tooltip content={<CustomTooltip D={D} />} />
                  <Bar dataKey="revenue" fill="#C9A14A" radius={[6, 6, 0, 0]} fillOpacity={0.85} />
                </BarChart>
              </ResponsiveContainer>
            ) : (
              ...
              <ResponsiveContainer width="100%" height={160}>
                <PieChart>
                  <Pie data={PIE_DATA || []} cx="50%" cy="50%" innerRadius={45} outerRadius={70} dataKey="value" strokeWidth={2} stroke={D.card}>
                    {(PIE_DATA || []).map((d, i) => <Cell key={i} fill={d?.color || "#C9A14A"} />)}
                  </Pie>
                  <Tooltip />
                </PieChart>
              </ResponsiveContainer>
          ...
          chartData.length > 0 ? (
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={chartData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                ...
                <YAxis tick={{ fontSize: 11, fill: D.muted }} axisLine={false} tickLine={false} tickFormatter={fmtShort} width={55} />
                <Tooltip content={<CustomTooltip D={D} />} />
                <Line type="monotone" dataKey="revenue" stroke="#C9A14A" strokeWidth={2} dot={false} activeDot={{ r: 4, fill: "#C9A14A" }} />
              </LineChart>
            </ResponsiveContainer>
          ) : (
        ...
        <div style={{ ...styles.card(D), overflow: "hidden", display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ overflowY: "auto", flex: 1 }}>
            {safeMessages.map(m => m && (
              <div key={m.id} onClick={() => { setActiveMsg(m); markRead(m.id); }} style={{ padding: "14px 16px", borderBottom: `1px solid ${D.border}`, cursor: "pointer", background: activeMsg?.id === m.id ? "rgba(201,161,74,0.08)" : "transparent", borderLeft: activeMsg?.id === m.id ? "3px solid #C9A14A" : "3px solid transparent" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                  ...
                  <div style={{ flex: 1, overflow: "hidden" }}>
                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                      ...
                      {m.unread && <span style={{ width: 8, height: 8, borderRadius: "50%", background: "#C9A14A", display: "inline-block" }} />}
                    </div>
              ...
              <div style={{ flex: 1, overflowY: "auto", padding: 20, display: "flex", flexDirection: "column", gap: 12 }}>
                ...
                {(activeMsg.replies || []).map((r, i) => (
                  <div key={i} style={{ alignSelf: "flex-end", maxWidth: "75%" }}>
                    <div style={{ background: "linear-gradient(135deg, #C9A14A, #e0b96a)", color: "#fff", padding: "12px 16px", borderRadius: "16px 0 16px 16px", fontSize: 13, lineHeight: 1.5 }}>{r}</div>
                  </div>
                ))}
              </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
        {[
          { label: "Total Earned", value: fmtShort(total), icon: IndianRupee, color: "#C9A14A", bg: "rgba(201,161,74,0.1)" },
            ...
            <tbody>
              {payments.map(p => (
                <tr key={p.id} style={{ borderBottom: `1px solid ${D.border}` }} onMouseEnter={e => e.currentTarget.style.background = D.bg} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                  <td style={{ padding: "14px 16px", fontSize: 13, fontWeight: 600, color: "#C9A14A" }}>{p.id}</td>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "flex", gap: 24, borderBottom: `2px solid ${D.border}`, marginBottom: 8 }}>
        <button onClick={() => setActiveTab("reviews")} style={{ padding: "8px 0", fontSize: 14, fontWeight: 600, color: activeTab === "reviews" ? "#C9A14A" : D.muted, borderBottom: activeTab === "reviews" ? `2px solid #C9A14A` : "none", background: "none", border: "none", cursor: "pointer" }}>
          My Reviews ({reviews.length})
        </button>
        <button onClick={() => setActiveTab("widget")} style={{ padding: "8px 0", fontSize: 14, fontWeight: 600, color: activeTab === "widget" ? "#C9A14A" : D.muted, borderBottom: activeTab === "widget" ? `2px solid #C9A14A` : "none", background: "none", border: "none", cursor: "pointer" }}>
          Review Widget
        </button>
        <button onClick={() => setActiveTab("questions")} style={{ padding: "8px 0", fontSize: 14, fontWeight: 600, color: activeTab === "questions" ? "#C9A14A" : D.muted, borderBottom: activeTab === "questions" ? `2px solid #C9A14A` : "none", background: "none", border: "none", cursor: "pointer" }}>
          Question & Answer
        </button>
        <button style={{ marginLeft: "auto", fontSize: 13, color: "#C9A14A", background: "none", border: "none", cursor: "pointer" }}>+ Add View</button>
      </div>
              ...
              <div key={r.id} style={{ ...styles.card(D), padding: 0, position: "relative", overflow: "hidden" }}>
                ...
                <div style={{ padding: "20px 20px 16px 44px" }}>
                  ...
                  <div style={{ marginBottom: 12 }}>
                    ...
                    <div onClick={() => handleProductClick(product.id)} style={{ fontSize: 14, fontWeight: 600, color: "#C9A14A", cursor: "pointer", marginTop: 2 }}>{product.name}</div>
                  </div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        <div style={{ ...styles.card(D), padding: 24 }}>
          <div style={{ fontWeight: 600, fontSize: 15, marginBottom: 18, display: "flex", alignItems: "center", gap: 8 }}><User size={16} color="#C9A14A" /> Profile Settings</div>
        ...
        <div style={{ ...styles.card(D), padding: 24 }}>
          <div style={{ fontWeight: 600, fontSize: 15, marginBottom: 18, display: "flex", alignItems: "center", gap: 8 }}><Lock size={16} color="#C9A14A" /> Security</div>
        ...
        <div style={{ ...styles.card(D), padding: 24 }}>
          <div style={{ fontWeight: 600, fontSize: 15, marginBottom: 18, display: "flex", alignItems: "center", gap: 8 }}><Bell size={16} color="#C9A14A" /> Notification Preferences</div>
          {Object.entries(notifPrefs).map(([k, v]) => (
            <div key={k} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 0", borderBottom: `1px solid ${D.border}` }}>
              ...
              <button onClick={() => setNotifPrefs(p => ({ ...p, [k]: !p[k] }))} style={{ background: "none", border: "none", cursor: "pointer", padding: 0 }}>
                {v ? <ToggleRight size={28} color="#C9A14A" /> : <ToggleLeft size={28} color={D.muted} />}
              </button>
            </div>
          ))}
        </div>
        ...
        <div style={{ ...styles.card(D), padding: 24 }}>
          <div style={{ fontWeight: 600, fontSize: 15, marginBottom: 18, display: "flex", alignItems: "center", gap: 8 }}><Zap size={16} color="#C9A14A" /> Appearance</div>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 0", borderBottom: `1px solid ${D.border}` }}>
            ...
            <button onClick={() => setDark(p => !p)} style={{ background: "none", border: "none", cursor: "pointer", padding: 0 }}>
              {dark ? <ToggleRight size={28} color="#C9A14A" /> : <ToggleLeft size={28} color={D.muted} />}
            </button>
          </div>
...
const EmptyState = ({ icon: Icon, title, sub }) => (
  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "60px 0", gap: 10 }}>
    <div style={{ width: 64, height: 64, borderRadius: 20, background: "rgba(201,161,74,0.1)", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <Icon size={28} color="#C9A14A" />
    </div>
  ...
  if (active && payload && payload.length) {
    return (
      <div style={{ background: D?.card || "#fff", border: `1px solid ${D?.border || "#e2e8f0"}`, borderRadius: 10, padding: "10px 14px", boxShadow: "0 4px 20px rgba(0,0,0,0.1)" }}>
        ...
        {payload.map((p, i) => (
          <div key={i} style={{ fontSize: 13, fontWeight: 600, color: p.color || "#C9A14A" }}>{fmtShort(p.value)}</div>
        ))}
      </div>
    );
  }
